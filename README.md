...
Every major subsystem was built around the same field work idea. The binary compaction engine exists because we only have 8mb of flash to work with in LittleFS, and I want to survive long runtimes. The companion app exists because mobile GPS, relay capability, and operator input make the stationary network smarter. The custom encrypted BLE layer exists because the phone link is meant to be operational without becoming an Android developer. The UI theme exists because its fun, and cool. It also let me use LVGL for the first time - TFT is out.
