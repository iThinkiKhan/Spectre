...
private:
    ...
    // One counter per secure channel.  Sized for channels 1..15 with headroom
    // beyond the current top channel (LOG_STREAM=8) so adding new channels
    // doesn't risk an out-of-bounds index.
    uint32_t _txCounter[16] = {};
    uint32_t _rxCounter[16] = {};
};
