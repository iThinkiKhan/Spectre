...
void LVGLDriver::tick() {
    // Auto-measure the elapsed time so LVGL's tick stays aligned with the
    // wall clock regardless of how often this is called. The previous
    // hardcoded `lv_tick_inc(1)` only matched the contract when callers
    // ran every 1 ms exactly — anything coarser left animations running
    // proportionally slower than intended. (The real render loop in
    // TaskDisplay bypasses this helper and feeds lv_tick_inc(delta)
    // directly, so this self-correction is purely defensive.)
    ...
    const uint32_t delta = now - s_lastMs;
    s_lastMs = now;
