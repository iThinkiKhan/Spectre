...
public:
    ...
    bool requestUploadStop(const char* reason = nullptr);
    bool requestUploadResume(const char* reason = nullptr);
    ...
    bool      dumpAvailable();
    int       queueDepth();
    uint32_t  lastDumpAge() const { return millis() - _lastDumpMs; }
    ...
    bool      uploadStoppedBySerial() const { return _uploadStoppedBySerial; }
...
private:
    ...
    struct DumpContext {
        ...
        uint32_t lastCheckpointPublished = 0;
    ...
    bool          _stopCleanup = false;
    bool          _uploadStoppedBySerial = false;
    ...
    StorageExclusiveWindow _storageWindow;
