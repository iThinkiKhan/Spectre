...
static void _workerTask(void*) {
    ...
    for (;;) {
        ...
        if (!_popNextPendingSlot(idx)) {
            ...
            // Bound the idle wait so the post-batch metadata-flush check
            // (dueByTime at STORAGE_HOT_META_SAVE_INTERVAL_MS) still runs
            // when captures stop. With portMAX_DELAY the worker would only
            // re-evaluate flush conditions on the next enqueue, leaving
            // dirty metadata stranded during quiet periods.
            ulTaskNotifyTake(pdTRUE,
                             pdMS_TO_TICKS(WORKER_IDLE_FLUSH_RETRY_MS));
            continue;
        }
        ...
        while (morePending) {
            ...
            if (_isMigratedKind(kind)) {
                ...
                batchTiming.checkpointMs += recordTiming.checkpointMs;
                ...
                if (r.ok() && elapsed >= WORKER_SLOW_MS) {
                    DLOG_WARN("STORAGE",
                              "queued write slow type=%s total=%lu parse=%lu classify=%lu build=%lu append=%lu checkpoint=%lu index=%lu counter=%lu ui=%lu",
                              _slotKindText(kind),
                              static_cast<unsigned long>(elapsed),
                              ...
                              static_cast<unsigned long>(recordTiming.checkpointMs),
        ...
        if (processed > 0) {
            ...
            if (batchSlow) {
                DLOG_WARN("STORAGE",
                          "queued write slow batch records=%lu total=%lu parse=%lu classify=%lu build=%lu append=%lu checkpoint=%lu index=%lu counter=%lu ui=%lu close=%lu ok=%d",
                          ...
                          static_cast<unsigned long>(batchTiming.checkpointMs),
...
bool tryPauseWorkerIfIdle(uint16_t maxInflight) {
    if (!isReady()) {
        return false;
    }
